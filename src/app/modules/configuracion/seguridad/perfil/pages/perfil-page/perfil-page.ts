...
export class PerfilPage implements OnInit, OnDestroy {
  ...
  onAddPerfil(): void {
    ...
    dialogRef.afterClosed().subscribe((result: IPerfilCreateUpdateRequest | undefined) => {
      if (result) {
        this.isLoading.set(true);
        this.perfilService.crearActualizarPerfil(result).pipe(tap((response) => {
              this.showSnackbar(
                response.vMensaje || 'Perfil creado exitosamente.',
                'snackbar-success'
              );
              this.cargarPerfiles();
            }),
			...
			dialogRef.afterClosed().subscribe((result: IPerfilCreateUpdateRequest | undefined) => {
				if (result) {
					...
					this.perfilService.crearActualizarPerfil(result).pipe(
						tap(response => {
							this.showSnackbar(response.vMensaje || `Perfil "${perfil.vPerfil}" actualizado.`, 'snackbar-success');
							this.cargarPerfiles();
						}),
						catchError(error => {
      ...
      if (result && result.confirmado) {
        ...
        this.perfilService
          ...
          .pipe(
            tap((response) => {
              this.showSnackbar(
                response.vMensaje || `Perfil "${perfil.vDescripcion}" desactivado.`,
                'snackbar-warn'
              );
              this.cargarPerfiles();
            }),
